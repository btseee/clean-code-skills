...
- ordering functions so the file reads top-down: callers above callees, each level descending one
  step in detail
...
- Use plain data structures for plain data; use objects, records, or types to protect real
  invariants. The two are opposites: an object hides data and exposes behavior; a data structure
  exposes data and implies no behavior. Code that treats one as the other is where "anemic" and
  "god object" designs both come from.
- Ask collaborators for behavior instead of navigating through their internals (`a.b().c().d()`
  chains couple you to every link).
...
- model expected alternate outcomes (not found, empty, declined) as values, result types, or
  special-case objects — reserve exceptions for genuine failures
...
2. No duplicated knowledge — one authoritative home per rule or fact. Confirm the duplication is
   *true* duplication first; copies that change for different reasons at different rates are not
   duplicates, and merging them is harder to undo than leaving them apart.
